...
export function BlogContent() {
  const [posts, setPosts] = useState<BlogPostDisplay[]>(fallbackBlogPosts as any);
  ...
  const loadBlogData = async () => {
    try {
      // Load only essential data - 12 posts max
      const [postsResponse, categoriesResponse] = await Promise.all([
        blogService.getPublishedPosts({ page: 0, limit: 12 }).catch(() => ({ results: [] })),
        ...
        {hasMore && (
          <div className="text-center mt-12">
            <Button 
              ...
              disabled={isPending}
            >
              {isPending ? 'Đang tải...' : 'Tải thêm bài viết'}
            </Button>
          </div>
        )}
      </div>
